...
source ./src/utils/color-palette.sh
...
serve_multi_choice_menu(){
    ...
    trigger=true
    prompt="Check program to install (type index again to uncheck, ENTER when done or X to exit): "
    ...
    if [ "$trigger" == "true" ]; then
        ...
        #     fi
        # done
        # 1st attempt for sourcing programs to install
        for i in ${!choices[@]}; do
            if [ "${choices[i]}" == "*" ]; then
                read -ra array_of_map_values <<< "${options[${keys[i]}]}"
                source_script_for_execution "${array_of_map_values[0]}" "${array_of_map_values[2]}"
            fi
        done
